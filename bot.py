...
async def _transform_llm_ok(given, card):
    ...
    system = ("Ты — справедливый, но СТРОГИЙ проверяющий грамматику. Дан исходник в Present "
              "Simple, целевое ВРЕМЯ и эталон. Ответь ОДНИМ словом: YES, если предложение "
              "ученика — корректная форма исходника в нужном времени. Мелкие опечатки, синонимы "
              "и пунктуация — ОК, ЕСЛИ время и форма глагола верны. НО неверное время или "
              "неверная форма глагола (даже на одну букву, напр. «build» вместо «built») — NO.")
...
def _transform_exact_ok(given, expected):
    """GR2 exact-грейдер: СТРОГО нормализация (регистр/хвостовая пунктуация/пробелы), БЕЗ
    допуска-на-опечатку. Для трансформации времён одна буква формы глагола (build/built) —
    это сама грамматика, а не опечатка; _one_edit_away здесь ложно засчитывал near-miss.
    Толерантность к реальным опечаткам/синонимам (при ВЕРНОЙ форме) — на узком LLM-чеке."""
    def norm(s):
        s = (s or "").strip().lower()
...
async def on_flip(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    word = db.get_word(wid)
    if not word:
        await q.answer("Слово не найдено", show_alert=True)
        return
    # stale-tap guard: flip должен соответствовать текущей карточке очереди
    queue = ctx.user_data.get("review_queue", [])
    pos   = ctx.user_data.get("review_pos", 0)
    if not queue or pos >= len(queue) or queue[pos] != wid:
        await q.answer("Карточка устарела — нажми ☀️ Повторить", show_alert=True)
        return
    ...
    block = ("\n" + _network_block(word)) if _variant(uid, wid) == "layered" else ""
    await q.edit_message_text(
        f"{label} · карточка {pos + 1}/{len(queue)}\n\n"
